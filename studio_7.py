...
def main():
    ...
    average_salaries_by_industry = get_average_salary(industry_groups)
     ...
     #filter by more than 10
    filtered_average_salary_by_industry = []

    for entry in average_salaries_by_industry:
        if entry.participant_count >= 10:
            filtered_average_salary_by_industry.append(entry)


    #sorting logic
    filtered_average_salary_by_industry.sort(key=lambda x: x.average, reverse=True)

    top_5_indutries = []
    for entry in filtered_average_salary_by_industry[:5]:
        top_5_indutries.append(entry.key)

    print("Answer #2" , top_5_indutries)
    print("....................")
    print("Answer #3")
    ...
    education_groups_average_salary = get_average_salary(education_groups)

    for group in education_groups_average_salary:
        print(group.key, group.average)
    ...
    experience_groups_average_salary = get_average_salary(experience_groups)

    for group in experience_groups_average_salary:
        print(group.key, group.average)
